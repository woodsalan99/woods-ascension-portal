...
| D28 | **Call classification keys on `answered`, NOT `keypad_entries`** — superseding D25. Settled by a real end-to-end test (Alan called the tracked number, pressed 1, connected): CallRail returns `keypad_entries: null` on every call including that one, so the keypress is simply not exposed to the API. `answered: true` (recording present) is what actually distinguishes a call that cleared the menu and connected to TalkRoute. Unanswered calls are filed `MISSED` with `needsReview`, not silently binned as robocalls. | Two prior guesses (keypress-based, then answered-based, then back to keypress) were all made without evidence; the test call produced it. Known conservative gap logged in code: a caller who presses 1 but isn't picked up is recorded but doesn't raise a lead notification | 08-02 |
| D29 | Gmail integration scope extended to include `gmail.send`; parsed leads are forwarded to the client as a clean summary email in addition to the Pushover alert. `notify()` generalised from Pushover-only to a channel fan-out (PUSHOVER always, EMAIL only when the caller supplies an `emailBody`), sharing one `active` flag and one delivery log. | Alan's explicit choice of a readable summary over a raw forward; EMAIL was already a stubbed channel type in the schema, so this filled it in rather than adding a parallel mechanism | 08-02 |
| D30 | LSA email parsing rebuilt against REAL messages after discovering the original matcher captured nothing at all (it tested the subject for a phrase that only appears in the body). Google sends two shapes: a structured new request, and a "customer sent you a message" variant that typically carries the customer's phone number — both now parsed, with `needsDetails` set only when no number was supplied. | The original parser was written from the handoff's prose description with no real sample; validating against Alan's actual inbox was the first time it was checked | 08-02 |
| D31 | **One person = one lead.** All inbound contacts (call, text, website form, LSA request) route through `recordContact()` in `src/lib/lead-identity.ts`, which merges onto an existing lead or creates one, and always writes a `LeadActivity` timeline entry. Match order: CallRail `personId` → normalized phone (digits, last 10) → exact name, the last ONLY when phones don't contradict and never for the LSA placeholder "Potential Customer". Enrichment fills empty fields only, never overwrites. `LeadActivity.dedupeKey` (unique) makes re-syncs idempotent. CallRail texts (`/text-messages.json`) are logged to the timeline but never create a lead on their own — the tracking number receives significant cold-outreach spam. | Alan's requirement that repeat contacts merge and accumulate history. CallRail's own `personId` turned out to already provide cross-contact identity (same value across a person's calls and texts) and was simply not being read. Name-only matching is deliberately the weakest signal because two customers can share a name | 08-02 |
| D32 | Missed calls are logged to an existing lead's timeline but never create a lead or raise a notification. The timeline entry explicitly states the keypress is unknowable rather than implying it. | Alan asked for unanswered-but-pressed-1 calls to be annotated; CallRail returns `keypad_entries: null` on every call including ones that demonstrably pressed and connected, so that distinction cannot be made. Stating the limit in the UI beats silently guessing | 08-02 |
| D33 | Overview defaults to a **rolling last-30-days** window, with a one-tap switch to month-to-date. The period grammar in `ls-metrics.ts` gains `last30` alongside `YYYY-MM`; `periodRange()` handles both, so every row-counting resolver honours it for free. LSA figures are the exception — Google publishes them monthly only, so on `last30` the resolver uses the newest month on file and names that month in the support line rather than pro-rating. Work items now carry an optional `date`, which is what ages them out of the rolling view; undated items count as month-end. | Alan: month-to-date "looks naked and weak on the early days of the month". Measured on Aug 3: month-to-date showed 2 customers and no cost-per-lead at all; last-30 showed 7 customers and $19/lead. Pro-rating the ad spend was rejected as inventing a number Google never published | 08-03 |
| D34 | Leads are **soft-deleted**, worthwhile **by default**, and shown as "Unknown Name" when the name is missing. `ServiceLead.deletedAt` is a tombstone: CallRail's cursor is inclusive, so the same call is re-fetched every run and a hard delete would simply reappear. `findExistingLead()` therefore matches tombstones too, and `recordContact()` restores a deleted lead only when the incoming event is genuinely new — a re-sync of an already-logged event leaves it deleted. `qualified` inverts: `false` = bad fit (the exception), `null`/`true` = counts. Every lead-counting resolver excludes both deleted and bad-fit rows. | Alan: "Woods Connie, for example, was me" — he needs to remove his own test leads. And "all leads should be set as worthwhile by default, because there is so much filtering that happens upfront; the action should be to mark leads as Bad Fit instead". "Name hidden by Google" was replaced because it "sounds bad" to a client reading their own board | 08-03 |
| D35 | **A lead is never called a customer**, anywhere in the portal. The portal knows who got in touch; it has no idea who paid. Every client-facing "customers" string became "leads", and the Monthly Recap now lists each lead behind the headline number — name where known, source, service, town, date, and the job value only once it's marked won. The Numbers page also gains the full period picker (last 30 days by default, month-to-date, the three previous months, last 90 days); the "since we started" chart deliberately ignores it. LSA figures aggregate whole published months across a rolling window — summed for spend/leads/impressions, impression-weighted for rates — and the support line names the months used. | Alan: "we have zero idea whatsoever if they closed. You MUST differentiate between leads and customers. We don't HAVE their customer data." The per-lead breakdown was his follow-on: a bare "6 leads" asks for trust, a named list can be checked against what he and Bryan actually remember | 08-03 |
| D36 | **Watchdog cron** at `/api/cron/watchdog`, alerting Alan only, never the client. Three checks: a sync source that hasn't succeeded within a generous multiple of its interval (45m for the 5-minute ones, 36h for the daily ones), a client with zero leads in 14 days that has had leads before, and leads sitting in NEW for over 48 hours. Each alert carries a signature and re-fires at most once per 20 hours. | Every other cron reports its own failures; none can report not running at all. A crashed service, a deleted schedule or an expired token produces silence, and silence is indistinguishable from a quiet week. `WATCHDOG` had been a documented notification kind since Phase 1 with nothing ever emitting one | 08-03 |
| D37 | The Numbers' status badges ("Excellent", "Normal", "Room to grow") moved into the content registry and are hidden whenever the metric resolves to a blank or a dash. | They were hardcoded literals, which broke the local-services rule that nothing client-facing is a hardcoded string — Alan couldn't edit them. Worse, once the period picker landed (D35) a card could show "—" for a month with no figures entered while still asserting "Excellent" beside it. The chart's per-month lead counts were also missing the deleted/bad-fit filter every other lead count uses, so a deleted lead stayed visible in it | 08-03 |
| D38 | **Phone pass**, done by rendering the real stylesheet against real markup at 375px and 320px rather than reasoning about it. Fixed: the period switch's date range was a pill identical to the buttons beside it and read as another option (now plain caption text on its own line); the recap's 30px side padding squeezed its KPI grid to one column, and an odd cell count left a grey hole in the corner (18px padding, two columns, last-of-odd spans full width); the task urgency pill sat beside the title and crushed it to four lines (now below it); stacked keyword tabs lost all affordance once wrapped (now bordered). Verified zero horizontal overflow at both widths. **Copy pass**: "organic", "move the needle", "compound", "lever", "the campaign" and a duplicated "The numbers" eyebrow all replaced with what a painter would actually say. | Alan: "do the full copy and mobile pass like a human would that's not so techy" — Bryan is phone-first, and jargon in a portal he reads weekly is a slow tax on trust | 08-03 |
| D39 | Prose content limits raised from 300–500 to 1500 characters, a live character counter added beside every edit field, and multiline copy now renders with `white-space: pre-line` so a blank line becomes a real paragraph break. | Alan hit the 500-char cap rewriting the thesis (his text was 563) and only found out on save, after the writing was done. The limits were guesses meant to protect layout — true for a short label on a card, not for a body paragraph. The bigger problem was behind it: even once saved, HTML collapses newlines, so his two paragraphs would have rendered as one wall of text | 08-03 |
| D40 | Overview reworked on Alan's read-through: a second emphasis marker `__bold__` alongside the existing gold `**` (the thesis stylesheet renders `b` as gold italic, so there was no way to write a genuinely bold lead-in); the 30-second summary gained a visible Expand/Shrink caret, since hiding the native `<details>` marker had left it reading as a plain heading; the reviews KPI links to the real Google Maps profile; "Needs you" became "Latest leads", showing the six most recent with editable name and notes that write straight to the same lead record; and the junk-blocked card was removed. | "Needs you" only ever listed leads with a next-action date set, which almost none have, so the busiest page's main block was nearly always empty. The junk card's counting was correct but everything it had ever counted was two of Alan's own form tests and zero robocalls — a headline made of nothing. The `junk.blocked` resolver stays for when there's real volume | 08-03 |
| D41 | Client tasks are authored from the client's own page in edit mode, not the admin panel: add, edit, reorder, free-text urgency label, and an optional `ClientTask.dueAt` deadline rendered as "Due in 3 days" / "2 days late". Weekly habits render formatted whenever the editor is closed. | Alan: "I shouldn't need to go to admin for this" — he reads the portal in preview far more than he opens the admin panel, and the moment he decides they need to do something is while he's looking at the page. Separately, the habits block branched on `isPreview` rather than `editing`, so merely previewing showed the raw `📞\|Pick up…\|…` pipe strings instead of the formatted list | 08-03 |
| D42 | Where You Rank restructured around **the three assets** a homeowner can find them through — a "core pieces" band (1 Maps, 2 Website, 3 Google Ads), then one titled section per asset, each with its own collapsible "What controls your position". The "Are we improving?" month strip and the old flat factor grid are gone. The website asset stops being a black box: monthly visits-from-Google bars, times-appeared, pages showing, and Ahrefs keyword positions with search volume and movement. Google Ads visibility metrics moved here from The Numbers. | Alan: the website side "is certainly a bit of a black box to the client's view, which I don't want" — it had been one badge reading "34 visits · 3,711 times shown" beside a list of page names. Organising by asset rather than by which tool reports what matches how a painter actually thinks about where customers come from | 08-03 |
| D43 | The Numbers leads with a gold-outlined trio — total leads, leads from Google Ads, leads that cost nothing — then jobs and reputation beneath. `jobs.won` renders "Not tracked yet" instead of `0` until the first lead is ever marked Job Won, with a support line saying why. The reviews status badge was removed. The since-we-started chart now shows the free/paid split under each month's total, not just a total. | Alan: those three lead numbers "are the core focus"; a flat `0` on Jobs Won "looks bad" when the truth is nobody has marked an outcome yet — a zero we can't stand behind is worse than a blank; and "Room to grow" was wrong on 34 reviews at a perfect 5.0 | 08-03 |
| D44 | An unnamed lead shows its **phone number**, then its email, and only falls back to "Unknown Name" when we have neither. One helper (`src/lib/lead-label.ts`) used by the leads board, the lead detail sheet, the Overview's Latest Leads and the recap breakdown. | "Unknown Name" is identical on every unnamed card, so a board with three of them gives the client nothing to tell them apart by — while the number is the thing they actually recognise and can act on. Verified against production: all 18 live leads now show a name or a number, none reach the placeholder | 08-03 |
| D45 | Overview lead rows show the lead's **words**, not a note count: newest note (falling back to the enquiry message) as a snippet while collapsed, and the full message plus every note when expanded. Where You Rank moves the how-to-read-the-map explanation inside Asset #1 where it belongs, centres the three asset headings, and titles each controls panel by asset. Google Ads metrics render on both The Numbers and Where You Rank. | Alan spotted the 727 lead showing no notes on the Overview while the Leads page clearly had detail for it — the cause was that these leads have zero `LeadNote` rows and their detail lives in `ServiceLead.message`, which only the Leads page read. "What controls your position" appearing three times running never said which position. Ads on both pages answers two different questions from one set of resolvers, so they can't disagree | 08-03 |
| D46 | Search visibility renders as one chart — visits as bars, times-appeared as a line on its own scale (impressions run ~100x clicks, so a shared axis flattens the bars) — on both Where You Rank and The Numbers. Keyword positions are stated in words first ("5th result on page 1") over a position track, replacing a bare number in a circle. The town-page list collapses behind an Expand toggle. Asset #3 was rendering dashes because it asked for the current month, which Google hasn't published — it now uses the newest month on file and names it. `reviews.count` became period-aware. A new "Plan for MONTH" block on the Overview reads `MonthlyWork.nextMonth`. | Alan: the keyword block was "a bit confusing even for me", and Asset #3 "shows absolutely no data". Reviews showed the same 34 for May, June and July because the metric had no period and only two snapshots exist, both from August — past counts are genuinely unmeasured, so it now returns blank and the cell drops rather than asserting a number we never took | 08-03 |
| D47 | Review history reconstructed from what Google's profile reports in whole weeks ("2 / 7 / 38 weeks ago" against a live 34), stored as snapshots on the dates the count stepped: 32 from Nov 2025, 33 from 15 Jun, 34 from 20 Jul. Real month-over-month movement now exists — May 32, June 33, July 34. The plan is edited on the Overview itself via `savePlan`, writing the same `MonthlyWork.nextMonth` the recap reads. | Alan supplied the relative timings when told past counts were unmeasured; dates are ±3 days by construction, immaterial at the month granularity anything reads them at. And he couldn't find where to write the plan — it was three clicks deep behind a collapsed month in the recap builder, which is a verdict on the placement | 08-03 |
| D48 | **One chart with switchable series** replaces the two that existed. Every value moved into a hover tooltip; both scales are labelled (people left, views right); a month with no data plots `null`, not zero. Series: site visits, total/free/paid leads, search impressions, ad views — visits and total leads on by default. Blocks of period-scoped numbers now carry a timeframe chip naming the window or the published month. | The old pair printed total leads **underneath the ad-views bars**, which put free leads visually inside the ads story when the whole argument of the portal is that they're separate — Alan called this out exactly. Permanently-drawn values are also what made "4,567" land on top of "40". Plotting an unmeasured month as zero would draw a collapse that never happened | 08-03 |
| D52 | Google's LSA emails come in a **third variant** — "New call from a potential customer" — with none of the dash-labelled fields; it parsed as unreadable and produced a wrong "Google is hiding their number" alert. Now a `CALL` variant with the call time extracted. LSA notifications rewritten to Alan's wording (opens with "if you already answered, ignore this"), and Google's original email is no longer forwarded — it is tracking links and a Dublin footer around one sentence we already extract. Website-form notifications still attach the original, because there it is the customer's own words. | The first real lead through the system was a call, and exposed all three problems at once | 08-04 |
| D53 | **TalkRoute voicemail emails are parsed** (no API exists; the email carries the caller's number and a plaintext transcription). The voicemail adopts a phone-less Google Ads call lead within ±90 minutes and hands it the number; a voicemail processed first is found by the later call email through the normal phone merge. Notification carries the transcript and the number. | In the live test the voicemail email arrived two minutes BEFORE Google's call email, so Alan's proposed "check 2–3 min after the call" would have missed it — correlation has to work in both directions. LSA calls bypass CallRail, so this transcript is the only record of what the caller wanted | 08-04 |
| D54 | Overview redesigned around a mockup Alan brought (GPT-generated, discussed before building): a 3-item status strip, trend badges on all 4 KPIs (percent for leads/cost-per-lead, hard counts for pages/reviews — a percent on "19 of 20" reads worse than "+1"), "What we built" as checkmark cards, and a new "Campaign focus" card paired with the Plan — 2-3 themes drafted by DeepSeek at the moment the plan is saved (`MonthlyWork.focusAuto`/`focusManual`, mirroring the existing `heroTitleAuto`/`Manual` pattern), never on render. Latest-lead rows gained real card borders — the old flush list had no boundary between one lead and the next. `ls-trends.ts` computes 30-day-vs-prior comparisons outside the override-capable resolver layer, since these are dashboard deltas a client would never need to hand-correct. | Alan: "the more automations we have, the better" but worried about "maintaining a bunch of small things for each client" getting clunky — generating at save-time rather than per-render avoids both a new integration surface and a recurring cost. Caught during build: `SitePage.indexedAt` records when the SYNC first saw a page as indexed, not when Google actually indexed it, so a brand-new GSC connection would show "+19 pages" this month for pages indexed months ago — suppressed the same way the reviews backfill was, rather than assert history never measured | 08-04 |
| D55 | Leads redesigned around a second GPT mockup, fact-checked against the schema before building (all 8 stages and the review-request data model already matched; two proposed changes were rejected — no new "Estimate Scheduled" stage, and job value stays settable only at the JOB_WON transition, both by Alan's explicit call). Added: a search bar (matches name/phone/email/service/location/message) and a source filter, quick-action call/text/note icons on every card, a follow-up-date editor and a one-tap "Request a review" button inside the detail view, and a synthetic "Lead created from X" opening line on every card's timeline (render-only, derived from `receivedAt` + `source`, not a written row). The lead detail view moved from a centered modal to a right-side drawer (full-width on phone) — it reads as "more about the card you tapped" rather than an interruption; the delete and job-won confirmations stayed centered modals since only the main detail view was asked to change. Two new actions: `setLeadFollowUp` (the field existed in schema but had no client-facing UI before this) and `requestLeadReview` (wires the existing but previously-unused `ReviewRequest.leadId`). | Alan: "be sure it all aligns first, because this is [ChatGPT's] input, not mine" — checking the brief against real code caught that both rejected ideas were actual product decisions dressed up as mockup detail, so they went to Alan directly via `AskUserQuestion` instead of being silently built or silently dropped; that same check is what surfaced the two real build gaps (follow-up UI, review-request wiring) as this session's substance, with the drawer itself being pure styling | 08-04 |
| D56 | **Cross-client leak fixed.** Canencia's owners were notified about another client's voicemail. Root cause: one shared inbox receives mail for every client, and `matches()` only ever asked "is this an LSA/TalkRoute email?", never "is it *this client's*?" — so the single LOCAL_SERVICES integration claimed everything of a known kind. Split the question in two: `matches()` (kind) and a new `belongsToClient()` (tenancy), the latter **failing closed** — no configured identifier means no claim. `GmailMeta` gained `recipients`, every routing header (To/Cc/Delivered-To/X-Forwarded-For/…) joined, since Google addresses LSA mail to the client's own account (`canenciapainting@gmail.com`) and that survives forwarding. Anything of a known kind a client can't prove it owns is refused and reported to Alan only, so a mis-set identifier surfaces as an alert instead of silently eating real leads. Audited all four catch-points: **CallRail was already safe** (scoped server-side by `company_id`), **website forms were already safe** (matched on the client's own `formFromAddress`), **LSA and TalkRoute were both wide open**. | Verified against real mail, not reasoning: all 8 historical Canencia messages still claimed, the Pamalu voicemail refused. **TalkRoute remains unattributable by design** — it mails the shared inbox directly (`To:` is Alan, not the client) and names no business in the body, the leaked one saying only "Mailbox: Default Mailbox". So voicemail capture stays off until Alan either names the TalkRoute mailbox per client or forwards each client's voicemails to a distinct address; `talkrouteMailbox`/`talkrouteToAddress` support both. Refusing a real lead is recoverable, handing one client another client's customer is not — hence fail-closed | 08-05 |
| D57 | **Clients can now only ever receive `NEW_LEAD`.** `notify()` gained a `CLIENT_VISIBLE_KINDS` allowlist that gates client channels regardless of what the call site passes; Alan's own (`clientId: null`) channels still receive everything. Separately, a Gmail **404 on fetching a message is now treated as permanent, not transient**: `history.list` keeps reporting messages that have since been deleted, and the old code alerted on each one, firing repeatedly. Those are counted as `vanishedMessages` in the run result instead; genuine transient errors still alert and now name the message id. | Every internal call site *did* already pass `toClient: false`, and an audit confirmed Bryan and Desiree had received none of the 15 sync alerts — but `toClient` defaults to **true**, so the guarantee rested on nobody ever forgetting the flag. Alan: "I don't want them to be getting any 'admin' / internal type of notifications, ONLY lead notifies" — that's a rule about kinds, so it belongs in the fan-out, not in each caller. Verified by resolving real recipients for all 5 kinds × both flag values: NEW_LEAD reaches 3 client channels, every other kind reaches 0 even with `toClient: true` | 08-06 |
...
| **3 — Leads vertical** | CallRail + Gmail ingestion + spam classification + Pushover + kanban | ✅ LIVE AND VERIFIED (08-02) — deployed to production, all real credentials connected (CallRail, Gmail OAuth completed by Alan, DeepSeek, Pushover), both cron jobs running on schedule (sync-callrail */15min, sync-gmail */5min). Proven with REAL data, not synthetic: a live sync-gmail run processed 64 real inbox messages and correctly classified 3 real historical form-test submissions — one ambiguous "testing by alan" message correctly held for manual review (spamVerdict null) rather than auto-decided, two obvious test submissions (patterned 555-0123 numbers) correctly logged as spam, zero false leads created. Found and fixed two real bugs live in the process: a shared-CallRail-account cross-client data leak risk (fixed before any data leaked), and a crash in sync-gmail when a freshly-connected integration's matcher config isn't populated yet (now degrades gracefully instead of taking down the whole batch). Real calls/website leads will populate the Leads board as they occur — no more testing needed, this is genuinely running |
| **4 — Rankings + Numbers** | GSC + Places syncs; manual admin entry forms (LSA/geogrid/Ahrefs/SitePage) | ✅ DONE (08-03) — Where You Rank and The Numbers both live. Search Console connected and backfilled 364 real days (Apr 32 visits/5,001 impressions → Jul 46/4,637); 19 of 20 town pages confirmed showing in Google, Halawa the only one outstanding. Google Places connected: 34 reviews at 5.0. Five real Local Falcon scans loaded with their map images. Admin data-entry page at `/admin/clients/[id]/local` covers LSA monthly figures, geogrid upload (averages computed from the pasted grid, never typed), Ahrefs CSV paste, town pages, work log, recap builder and client tasks |
| **5 — What I Need From You + Recap** | Tasks/submissions/R2 uploads; WorkLog; recap builder; MonthlyWork | ✅ DONE (08-03) — task list with autosaving text and browser-side photo compression (webp under ~50KB before upload, so no object store was needed — see the TaskSubmission schema note); review-request queue; weekly-habits block. Monthly Recap page reads one MonthlyWork row per month with a live KPI grid and a per-lead breakdown, and May/June/July are drafted from real data only |
| **6 — Export/Import + launch** | Backup/restore; July backfill; watchdog armed; copy + mobile pass | ◐ IN PROGRESS (08-03) — backfill done (19 leads with full timelines, LSA May–July, GSC 364 days). `scripts/backup-db.sh` takes a full per-table CSV backup, verified against production (46 tables, 1.3MB). Watchdog armed at `/api/cron/watchdog` (D36). Remaining: copy pass and mobile pass, then Bryan and Desiree's logins on Alan's say-so |
