...
| D-I | **New deps to install when a phase first needs them:** `zod`, `googleapis`, `@anthropic-ai/sdk`, `@aws-sdk/client-s3`, `@aws-sdk/s3-request-presigner`. Pushover = plain `fetch`, no lib. New env vars (add to `.env.example` in the phase that introduces each): `ENCRYPTION_KEY`, `ANTHROPIC_API_KEY`, `PUSHOVER_APP_TOKEN`, `GOOGLE_OAUTH_CLIENT_ID`, `GOOGLE_OAUTH_CLIENT_SECRET`, `R2_ACCOUNT_ID`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, `R2_BUCKET`, `R2_PUBLIC_BASE_URL`. |
