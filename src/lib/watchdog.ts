...
// This is the only check that treats absence as the signal.
//
...
const MAX_SILENCE_MINUTES: Record<string, number> = {
  ...
  // The hourly Smartlead sync is also what triggers this check, so it can
  // never catch its own silence — that one is covered by Railway's own
  // deploy alerts, not here.
...
export async function runWatchdog(): Promise<{ raised: number; checks: string[] }> {
